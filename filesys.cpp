...
void FileStore::load_data(Address address, Block *block_data)
{
...
void FileStore::write_data(Address address, const Block *block_data)
{
    std::string full_path = address.database_name + "/" + address.file_name;
    FILE *file = fopen(full_path.c_str(), "rb+");
    if (file == NULL)
...
void FileStore::create_file(Address address)
{
    std::string full_path = address.database_name + "/" + address.file_name;
    FILE *file = fopen(full_path.c_str(), "w");
    fclose(file);
}
void FileStore::remove_file(Address address)
{
    std::string full_path = address.database_name + "/" + address.file_name;
